...
// upload 文件上传
func upload(ctx context.Context, fileName string, reader io.Reader, size int64) error {
	_, err := Client().PutObject(ctx, configs.BucketName, fileName, reader, size,
		minio.PutObjectOptions{ContentType: contentTypes[path.Ext(fileName)]})
	if err != nil {
		return err
	}
	return nil
...
// remove 文件删除
func remove(ctx context.Context, finalName string) error {
	return Client().RemoveObject(ctx, configs.BucketName, finalName, minio.RemoveObjectOptions{})
}

func RemoveVideo(ctx context.Context, finalName string) error {
	err := remove(ctx, configs.Video+finalName)
	return err
}

func RemoveCover(ctx context.Context, finalName string) error {
	err := remove(ctx, configs.Cover+finalName)
	return err
}
